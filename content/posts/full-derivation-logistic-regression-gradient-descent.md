...
Start from the single-example loss \(\tag{eq:single-loss}\):
\[
\ell^{(i)}(w,b)
= - \Bigl[
   y^{(i)} \log f^{(i)}
...
Substituting \(\tag{eq:dldf}\), \(\tag{eq:dfdz}\), and \(\tag{eq:dzdw}\) gives
\[
\begin{align*}
\frac{\partial \ell^{(i)}}{\partial w_j}
&= \frac{f^{(i)} - y^{(i)}}{f^{(i)}\bigl(1 - f^{(i)}\bigr)}
...
From the definition of $J(w,b)$ in \(\tag{eq:J}\):
\[
...
The first two factors are still \(\tag{eq:dldf}\) and \(\tag{eq:dfdz}\).  
We only need
\[
\frac{\partial z^{(i)}}{\partial b}
= 1.
...
\[
\begin{align}
w_j &:= w_j - \alpha \frac{\partial J}{\partial w_j}, \tag{eq:gd-wj}\\[3pt]
b   &:= b   - \alpha \frac{\partial J}{\partial b}. \tag{eq:gd-b}
\end{align}
\]

Substituting \(\tag{eq:dJdwj}\) and \(\tag{eq:dJdb}\):
\[
\begin{align*}
w_j &:= w_j
 - \alpha \left[
